...
- **同一モデルのN回多数決を「検証」と呼ばない**: 系統的バイアスが消えない。**cross-model一致も独立した証拠ではない**（★2026-08-23改訂）: 異なるモデルでも同じ検索結果に収束すれば独立でない。独立性はorigin_id（発行主体+原データ+版日付）で判定する
...
- **調査計画の承認（Phase 1.2）を飛ばして収集を走らせない**: 軽い事実確認1件・依頼自体が具体的で低コストな場合を除き、着手前に軸/ソース/モードを提示する
- **レポートを省略して提案だけ書かない**: Phase 5.0 の調査レポート（背景→調査結果→考察=推論トレース→まとめ）は必ずチャット提示する。**停止して承認を待つかは条件付き**（★2026-08-23改訂・SKILL.md 5.0参照）: 1.2でDecide以上を確定済みかつ想定から大きく逸脱していなければ、レポート提示に続けてそのまま提案本体へ進んでよい。それ以外（成果物モード未確定・高ステークス・想定から逸脱）は承認を待つ
...
- **アイディア出し・リサーチ・発散（Phase1〜4.5）に ownership/保守/運用コストを持ち込まない**: 「どう作るか・保守負担・build≠own」は **Phase D（設計）で人間（rio）が舵取り**する領域。選択肢生成の段階で工数の心配をして案を萎ませない。**（★2026-08-23明確化）ただしPhase 5.1（収束・2案への絞り込み）では実行可能性・機会費用・不可逆性を提案の記載事項として評価してよい**（さもないとDecideの意思決定にならない）。「持ち込まない」のは発散段階の話であり、収束段階の判断材料からownership/保守を除外する意味ではない
...
- [ ] Phase 5.0 調査レポート（背景/調査結果/考察=推論トレース/まとめ）をチャット提示したか（承認を待つかは条件付き。SKILL.md 5.0参照）
- [ ] ブラウザ取得を階層化したか（WebFetch既定→Claude in Chromeはメインagent限定→Playwright不使用）
- [ ] 全事実主張に検証可能なprovenanceがあるか（Web主張はURL付きソース、self-audit時はfile:line＋再現コマンド）
- [ ] source-verifier で claim検証済みか（ルールベース抽出→CoVe→一次ソース直接確認・origin_id独立性判定）
- [ ] grounded hallucination（NOT_ALIGNED）/ origin間の矛盾を洗い出したか
- [ ] **提案の根拠主張は2本以上の独立裏取りがあるか**（単一ソースは「裏取り不足」と明示）
- [ ] **主張の細部（数値/固有名詞/最上級）がソース範囲を超えていないか**（scope check）
- [ ] 「検証済み」に残存不確実性を併記したか
- [ ] Synthesisをcross-modelの競合仮説＋証拠適合度で検証したか（1サンプル確定・多数決でないか）
