...
**チャットが正・ファイルは控え**: 特に★の確認ポイント（1.2 計画承認 / 3.5 調査結果確認 / 4.5 発散レーン / 5.0 調査レポート）では、**rio が判断に必要な材料を全てチャット本文に書く**。「詳細は research.md / proposal.md 参照」とファイルへ丸投げして判断を求めない（rio にファイルを見に行かせない）。ファイルは記録・recall 用の控え。
