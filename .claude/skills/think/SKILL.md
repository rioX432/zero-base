...
2. **判定したらすぐ、Phase 1.2の表を作る前に**`${CLAUDE_SKILL_DIR}/references/profiles/_shared.md` を**明示的にRead**する（参照先は列挙しただけでは読み込まれない）。**`${CLAUDE_SKILL_DIR}/references/profiles/{profile}.md` はファイルが存在する場合のみRead**する（★2026-08-23修正: 以前「承認後にRead」としており、self-audit等のprofile固有dial（`default_endpoint`等）を知らないまま1.2の表を作ることになっていた。表を正確に作るため、承認前にRead する順序へ変更）。未実装4profileはファイルが無く、`_shared.md`の既定値のみで動く。
3. **Phase 1.2 の承認表に profile とその主要dial（primary_source/verification_mode/default_endpoint等、既定値から外れているもののみ）を明記し、rio の承認と同時に凍結する**（成果物ができてから自分に有利なルーブリックを選ぶ *rubric shopping* の防止）。
...
- **「全ソース並列」の意味（★2026-08-23明確化）**: これは「Decide以上ではどのソース種別（Gemini/Codex/SNS/Grok）が候補に入るか」を指し、**軸の数・各ソースへの呼び出し回数まで機械的に最大化する意味ではない**。呼び出し回数・エージェント数は`references/profiles/_shared.md`の「エージェント数/ツールコール数は問いの複雑性で決める」原則に従う（自明な軸に無駄な並列をかけない）。
...
> **allowed-tools の効き方（公式仕様・誤解しやすい）**: frontmatter の `allowed-tools` は「このスキルを起動した**ターンの間だけ**、許可を求めずに使えるツール」であり、**次のメッセージを送ると失効する**。/think は最大4箇所の停止条件（1.2/3.5/4.5/5.0、★2026-08-23改訂でリスク適応型に変更済み・毎回全て発火するとは限らない）を持ち、いずれかが発火すれば複数ターンにまたがる。**発火した場合、2ターン目以降はこの事前承認が効かない**。恒久的に許可プロンプトを避けたいツールは `.claude/settings.local.json` 側に置くこと。`Bash` はここに含めていない（無条件許可のリスクを避けるため。実行自体は都度承認で可能）。
...
4b. **`knowledge/{ドメイン}.md` を読む（レイヤーB=ドメインナレッジ）** — テーマが特定ドメイン（自社プロダクト等）に属する場合、該当ファイルを読む（例: Avvy関連テーマなら `knowledge/avvy.md`）。**プロダクトの定義・一次資料に接地した目標・明示された価値観や不可侵の制約・実装の地図・繰り返し踏んだ罠**を置く場所であり、profile.md と同じくrecall可。**ただし過去の /think が出した結論・診断・推奨は書かない・読まない**（それはレイヤーAでありanchoring毒。`workspace/INDEX.md` 側）。数値には必ず測定日を添える。
...
3. **ownership/保守は持ち込まない（★ただし発散フェーズ限定・2026-08-23明確化）**: 「どう作るか・保守・運用コスト・build≠own」は **Phase D（設計）で人間（rio）が舵取りする領域**。**Phase 1〜4.5（発散まで）**では理想解の探索に集中し、build工数/保守を理由に案を刈らない。**Phase 5.1（収束・2案への絞り込み）では話が別**: 実行可能性・機会費用・不可逆性は提案の記載事項として評価してよい（さもないとDecideで意思決定にならない）。「刈らない」のは発散段階の選択肢生成の話であり、収束段階の判断材料からownership/保守を除外する意味ではない。
...
### 1.2 ★調査計画の提示（★2026-08-23改訂・停止条件を先頭に明記）

**停止してrioの承認を待つ**のは、次のいずれかに該当する場合（Gemini Deep Research の collaborative planning 型。実測で高信頼な設計）:
- 調査範囲が広い・高コスト（Decide以上でLayer1.5を複数使う見込み等）
- 依頼が曖昧（軸・対象・成果物モードがテーマ文から一意に決まらない）

**上記に該当しない場合**（依頼自体が具体的・限定的で複雑度・コストが低い、または「軽い事実確認1件」で明らかに計画不要）は、計画を宣言した上でそのまま1.3へ続行してよい（rioが違和感を持てば1.3実行中でも止められる）。

提示する内容（簡潔な表1枚。続行する場合も宣言として必ず提示する）:
- **調査軸**（MECE分解の結果）
- 各軸に**使うソース**（Gemini/Codex/SNS/Grok のどれを回すか＝Layer1-3・全てブラウザレス。最重要claimはLayer1.5でClaude in Chrome Deep Researchを使うかも明記）
- **思考フレームワーク**（1.1aの選択）
- **成果物モード**（1.1の仮決め）と**終点**
- **effort**（並列ソース数・deep-researcher並列数の見積り）

停止条件に該当する場合 → rio が **承認 / 軸やモードを編集** → 確定してから 1.3 へ。
該当しない場合 → 提示に続けてそのまま1.3へ。
...
**Layer 1: Deep Search（全てブラウザレス・広さ担当）**
```
...
**Layer 1.5: Deep Research escalation（★2026-08-23改訂・最重要claimのみ・メインagent限定）**

Perplexity MCP(`perplexity-web`)は**恒常的な信頼性問題**があるため最重要軸専用の役割から外す（`perplexity_research`はMCPクライアントで頻繁にタイムアウト、`perplexity_ask`が`answer:null`を返す事例が本セッションでも複数回発生。Perplexity社自身が2026年3月に自社内でMCPから離脱する方針を表明）。代わりに、**Phase 2のclaim検証で「裏取り不足」と判定された最重要claim（1テーマ最大2-3件）に限り**、Claude in Chromeで実ブラウザのGemini Deep Research / ChatGPT Deep Research UIを操作し、数分かけた多段階リサーチをさせる。単発API呼び出しより深いが遅いため、**乱用しない**（並列MCP fan-outの代替ではなく補完）。メインagent限定（`references/verification.md`「一次成果物を直接取得」原則とも整合）。
...
- Layer 1.5（Claude in Chrome Deep Research）: 最重要claimのみ、1テーマ最大2-3件（メインagentの手動操作のため並列不可・時間コスト大）
...
1. **既定 = WebFetch / 検索API**（Gemini・Codex(web_search=live)・social-superpowers・grok）。全てブラウザレス。取得の大半はここで足りる。Perplexity MCPは信頼性問題により最重要軸専用の役割から外れた（Phase1.3 Layer1.5参照）。
2. **ログイン壁・JS重・403のページ、または最重要claimのDeep Research escalation = Claude in Chrome**（`mcp__claude-in-chrome__*`。rio の実ログイン済みセッションを使う。別プロセスのヘッドレス起動が不要）。**メインagent（自分）だけが実行**する — 対話認証MCPはサブagent（source-verifier/social-scanner）では使えないため、サブagentは当該URLを `NEEDS_BROWSER` として返し、メインagentが取得・再投入する。
...
2. **origin確認**（★2026-08-23改訂・以前は「複数ソース一致→高」だったが撤去。verification.md参照）: 複数ソースが見つかっても、遡って**同じorigin（発行主体+原データ+版日付）に収束していないか**を確認する。異なるoriginで一致 → 高、同一originの重複 → 中（1本と数える）、矛盾 → WebSearch/一次ソース直接取得で追加確認
...
4. **claim検証**: `source-verifier` agent で検証（`references/verification.md` P1-P2）。
   ...
   - **提案根拠になる重要claim・数値は、まず一次成果物を直接WebFetchで確認する**（★2026-08-23改訂・以前は「cross-modelで独立再検証」だったが撤去）。cross-model（Gemini/Codex/Grok）はURL発見・解釈比較の補助に限定し、モデル回答の一致を証拠として数えない。不一致は「論争あり」と明記。
   - **裏取り本数の必須化**（実測で単一ソース率93%が最大の弱点）: 提案の根拠主張は**単一origin禁止・最低2本の独立originによる裏取り**。取れなければ「裏取り不足」と明示し主柱にしない。
...
## Phase 3.5: ★ ユーザーとの調査結果確認（★2026-08-23改訂・条件付きゲート）

**必ず立ち止まる条件**: ①「0件」「反響なし」等の否定的結論が出た ②複数ソース間で矛盾がある ③高ステークス（Ship/Design直結、または金額・法令等の重要判断に関わる） ④Phase2/3で重大なGapが残っている。
**上記に該当しない場合**（順調に肯定的な結果が集まった等）、チャットで調査結果を**表形式で共有**した上でPhase4へ続行してよい。停止して承認を待つ必要はない（rioはいつでも介入できる）。

1. 調査結果を**表形式でチャットに共有**（続行する場合も省略しない）
2. 上記の立ち止まる条件に該当する場合は、ユーザーに検証を依頼してから先に進む
...
### 4.2 本質の抽出（★2026-08-23改訂: 多数決でなく競合仮説で1回勝負を避ける）
...
2. **異なるモデル**（Gemini/Codex のいずれか。ブラウザレス）に同じ材料で本質を出させる。
3. Claude案と異モデル案を**競合仮説として両方保存**する（一致=正しい、という多数決では扱わない。同じ材料からの一致は解釈の収束であって独立した証拠ではない）。
4. 一致点・相違点・各仮説を支持する証拠を列挙し、**証拠適合度**（どちらがより多くの検証済みclaimと整合するか）で本質を選ぶ。一致した場合も「解釈頑健性が高い」とだけ表現する。相違点は「不確実」と明記。
5. 同一モデルでN回回すのは独立性が偽物なので**しない**。

### 4.2a Understand終点の推論トレース（★2026-08-23追加・欠落を修正）

**Understandモードはここで終了するため**、Phase5.0まで進まず「事実→解釈→論点→選択肢」の推論トレースが生成されない欠落があった。Understandの場合は、4.2の本質抽出に続けて、5.0で使う表と同じ形式（研究結果に基づく事実→解釈→論点→分岐した選択肢）を**簡略版として本Phase内でチャットに提示**する（提案本体・却下理由の深掘りまでは不要。Decide以上に昇格する場合は5.0で正式版に差し替える）。
...
**→ チャットで発散案とprovenanceを提示する（★2026-08-23明確化: これは承認ゲートでなく非同期の提示。rioの応答を待たずPhase5へ続行してよい。rioが後から「これ深掘りして」と言えば、その時点でPhase3相当に差し戻す）。**

---

## Phase 5: Proposal（提案 + 反論検証）

### 5.0 ★調査レポート + 提案着手の確認（★2026-08-23改訂・条件付きゲート）

**いきなり2案を出さない＝レポートを省略して提案だけ書くことの禁止、は変更なし。** ただし「レポート提示後に必ず停止して承認を待つ」かどうかは条件付きにする:
- **停止して承認を待つ**: 1.2で成果物モードをDecide以上と確定していない／Phase3.5やPhase4.5で新情報が入りrioの想定と変わった可能性がある／高ステークス（Ship/Design直結）
- **停止せず続行してよい**: 1.2で既にDecide以上を明示承認済み、かつPhase2-4の結果が1.2時点の想定から大きく逸脱していない場合。この場合もレポート4節は必ずチャットに出力し、その直後に5.1へ続ける（rioはいつでも介入できる）

Phase 5 に入ったら、まず以下の構成の**レポートをチャットに出力**する。
...
上記「停止して承認を待つ」条件に該当する場合、レポート提示後に**AskUserQuestion で rio に確認**する: 「このまま提案に進む / 考察・論点を修正してから進む / 追加調査に戻る / ここで終了（Understand相当に降格）」。承認が出てから5.1へ進む。
該当しない場合は、レポート提示に続けてそのまま5.1へ進む（確認は求めない）。

このレポート（1〜3節）は `proposal.md` の冒頭にもそのまま置く。

### 5.1 提案生成（★5.0のレポート提示後。停止条件に該当した場合はrio承認後、該当しなければ続けてそのまま）
...
3. **生き残った反論を提案に反映して改訂**（Generator-Criticループ）。提案比較表を作成
   - **★counter-argument健全性チェック（2026-08-23追加・[EVAL-002](evals/EVAL-002-generator-critic-confirmation-device.md)）**: 改訂を終えたら、今回生き残った反論のうち**提案の主要な柱を棄却（不採用）したものが何件あったか**を数える。**複数ラウンドにわたって毎回0件（＝全反論が「改良」として吸収されただけ）が続いている場合、それはGenerator-Criticが結論を正当化する儀式になっている兆候として、チャットでrioに明示的にフラグを立てる**（実例: 2026-08-01 ai-agent-multiplex-2026では8件全てが改良吸収され、実測1回で提案の中核が崩壊した）。棄却0件そのものは悪ではないが、**実行して確かめられる論点があるなら、文章での反論より実測を優先する**
4. **judge 品質ゲート**: `judge` agent で採点（`references/verification.md` P4）。**judge は評価器であって判定者ではない。**
   ...
   - **`veto: true` が返ったら、スコアの高低にかかわらず即修正**（捏造引用・未解消のまま真として提示されたNOT_ALIGNED・単一origin主柱の未明示。開示・撤回済みのNOT_ALIGNEDはveto対象外）。平均で相殺しない。
